...
IESData_2017 <- read.csv("~/Desktop/R/IERP Data/IES_17_CM.csv")
IESData_2017$dinoflagellate_biomass <- as.numeric(IESCMData_2017$dinoflagellate_biomass)
...
ggplot()+
  ...
  geom_point(aes(x = Longitude, y = Latitude, color = dinoflagellate_biomass), data = IESCMData_2017)+
  ...
  ggtitle(expression("Chl-a Max Dinoflagellate Biomass 2017"))+
  labs(tag = "F")+
  theme(plot.tag.position = "topleft")

ggsave("IES_Dinoflagellate_CM_17.png", width = 6, height = 4, units = "in")